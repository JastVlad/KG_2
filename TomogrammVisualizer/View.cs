    ...
    class View
    {
        ...
        public void DrawQuadStrip(int layerNum, int min, int max)
        {
            ...
            for (int x_coord = 0; x_coord < Bin.X - 1; x_coord++)
            {
                ...
                value = Bin.array[x_coord + 0 * Bin.X + layerNum * Bin.X * Bin.Y];
                GL.Color3(TransferFunction(value));
                GL.Vertex2(x_coord, 0);

                value = Bin.array[x_coord + 1 + 0 * Bin.X + layerNum * Bin.X * Bin.Y];
                GL.Color3(TransferFunction(value));
                GL.Vertex2(x_coord + 1, 0);

                for (int y_coord = 1; y_coord < Bin.Y - 1; y_coord++)
                {
                    value = Bin.array[x_coord + 1 + (y_coord + 1) * Bin.X + layerNum * Bin.X * Bin.Y];
                    GL.Color3(TransferFunction(value));
                    GL.Vertex2(x_coord + 1, y_coord + 1);

                    value = Bin.array[x_coord + (y_coord + 1) * Bin.X + layerNum * Bin.X * Bin.Y];
                    GL.Color3(TransferFunction(value));
                    GL.Vertex2(x_coord, y_coord + 1);
                }
                GL.End();
